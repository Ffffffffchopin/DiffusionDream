...
def process_slices():
    ...
    for i in tqdm(range(10,len(images_list)),desc="Processing"):
        ...
        new_line = {'info':datasset_config.current_processing_bvid, 'current_frame':binary_to_base64(image_to_binary(images_list[i])),'action':action,'previous_frame_1':binary_to_base64(image_to_binary(images_list[i-10])),'previous_frame_2':binary_to_base64(image_to_binary(images_list[i-9])),'previous_frame_3':binary_to_base64(image_to_binary(images_list[i-8])),'previous_frame_4':binary_to_base64(image_to_binary(images_list[i-7])),'previous_frame_5':binary_to_base64(image_to_binary(images_list[i-6])),'previous_frame_6':binary_to_base64(image_to_binary(images_list[i-5])),'previous_frame_7':binary_to_base64(image_to_binary(images_list[i-4])),'previous_frame_8':binary_to_base64(image_to_binary(images_list[i-3])),'previous_frame_9':binary_to_base64(image_to_binary(images_list[i-2])),'previous_frame_10':binary_to_base64(image_to_binary(images_list[i-1]))}
